...
export default function fetchViberDb(limit) {
  const chatTokensParams = '?,'.repeat(chats.length).slice(0, -1);
  const chatTokens = chats.map((el) => el.Token);
  const sqlStmt = db.prepare(
    `SELECT ch.name as chName, m.EventID as eventId, c.ClientName as name, e.TimeStamp as timestamp, 
    ...
    WHERE ch.Token IN (${chatTokensParams})
  ...
  return sqlStmt.all(chatTokens, limit);
}

// console.log(fetchViberDb(5))
